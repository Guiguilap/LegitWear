...
const ANTHROPIC_API_KEY = "REMPLACE_PAR_TA_CLE_API";
...
const STYLES = `
  ...
  *,*::before,*::after{box-sizing:border-box;margin:0;padding:0}
  ...
  .hero-eyebrow{display:inline-flex;align-items:center;gap:10px;font-size:10px;letter-spacing:0.2em;text-transform:uppercase;color:var(--accent2);margin-bottom:28px;font-weight:400}
  .hero-eyebrow::before{content:'';display:inline-block;width:24px;height:1px;background:var(--accent2)}
  .hero h1{font-family:'Syne',sans-serif;font-size:clamp(36px,8vw,72px);font-weight:800;line-height:0.98;letter-spacing:-0.03em;color:var(--ink);margin-bottom:24px;max-width:680px}
  .hero h1 em{font-style:italic;color:var(--accent2)}
  .hero-sub{font-size:14px;color:var(--ink-soft);line-height:1.7;max-width:420px;margin-bottom:36px;font-weight:300}
  .hero-ctas{display:flex;gap:10px;flex-wrap:wrap}
  .hero-bottom{display:flex;gap:40px;padding-top:48px;border-top:1px solid var(--border);margin-top:48px}
...
// ─── COMPONENTS ──────────────────────────────────────────────────────────────
function Navbar({ user, onLogin, onSignup, onLogout, onDashboard, onHome, onShare, onSwitchAccount }) {
...
function Landing({ onLogin, onSignup, onShare, onCheckout, onCGU }) {
      ...
      <section className="hero">
        ...
        <div className="hero-inner">
          ...
          <div className="hero-ctas">
            ...
            <button className="btn btn-outline btn-lg" onClick={onLogin}>Se connecter</button>
            <button className="btn btn-ghost btn-lg" onClick={onShare}>🔗 Partager</button>
          </div>
        </div>
  ...
  const handle = () => {
    ...
    setTimeout(() => {
      ...
      if (mode === "signup") {
        ...
        users[email] = { email, password, name: name || email.split("@")[0], history: [] };
...
function Dashboard({ userEmail, onShare }) {
  ...
  return (
    <div className="cgu-page">
      ...
      <div className="cgu-section">
        ...
        <p className="cgu-p"><strong>LegitWear</strong> est un service d'aide à l'authentification de vêtements, sneakers et accessoires de mode basé sur l'intelligence artificielle. Le service est édité et exploité à titre personnel.</p>
        <p className="cgu-p">LegitWear n'est affilié à aucune marque mentionnée sur le site. Toutes les marques citées (Nike, Adidas, Supreme, Louis Vuitton, Gucci, etc.) appartiennent à leurs propriétaires respectifs.</p>
      </div>

      <div className="cgu-section">
        <div className="cgu-h2">2. Acceptation des conditions</div>
        <p className="cgu-p">En utilisant LegitWear, vous acceptez pleinement et sans réserve les présentes Conditions Générales d'Utilisation. Si vous n'acceptez pas ces conditions, veuillez ne pas utiliser le service.</p>
      </div>

      <div className="cgu-section">
        <div className="cgu-h2">3. Limitation de responsabilité</div>
        <p className="cgu-p"><strong>Service indicatif uniquement.</strong> Les analyses fournies par LegitWear sont générées par intelligence artificielle et ont un caractère purement indicatif et non contractuel.</p>
        <p className="cgu-p">LegitWear et son créateur <strong>ne peuvent en aucun cas être tenus responsables</strong> :</p>
        <ul className="cgu-list">
          <li>Des décisions d'achat ou de vente prises sur la base des analyses fournies</li>
          <li>Des pertes financières résultant de l'utilisation du service</li>
          <li>Des erreurs ou imprécisions dans les verdicts rendus</li>
          <li>De tout préjudice direct ou indirect lié à l'utilisation du service</li>
        </ul>
        <p className="cgu-p">LegitWear ne remplace pas l'expertise d'un professionnel certifié en authentification.</p>
      </div>

      <div className="cgu-section">
        <div className="cgu-h2">4. Propriété intellectuelle</div>
        <p className="cgu-p">L'ensemble des éléments constituant le site LegitWear (design, textes, logos, interface) est protégé par le droit d'auteur. <strong>Toute reproduction, copie ou utilisation sans autorisation écrite préalable est strictement interdite.</strong></p>
        <p className="cgu-p">© 2026 LegitWear — Tous droits réservés.</p>
      </div>

      <div className="cgu-section">
        <div className="cgu-h2">5. Données personnelles</div>
        <p className="cgu-p">Les données collectées (email, mot de passe, historique de scans) sont utilisées uniquement pour le fonctionnement du service. Elles ne sont ni vendues, ni partagées avec des tiers à des fins commerciales.</p>
        <p className="cgu-p">Conformément au RGPD, vous disposez d'un droit d'accès, de rectification et de suppression de vos données. Pour exercer ce droit, contactez-nous.</p>
      </div>

      <div className="cgu-section">
        <div className="cgu-h2">6. Photos uploadées</div>
        <p className="cgu-p">Les photos que vous uploadez sont utilisées uniquement pour l'analyse IA. Elles ne sont pas conservées de manière permanente ni utilisées à des fins commerciales.</p>
        <p className="cgu-p">Vous conservez l'entière propriété de vos photos.</p>
      </div>

      <div className="cgu-section">
        <div className="cgu-h2">7. Abonnements et paiements</div>
        <p className="cgu-p">Les abonnements sont gérés par <strong>Stripe</strong>, prestataire de paiement sécurisé. LegitWear ne stocke aucune information bancaire.</p>
        <ul className="cgu-list">
          <li>Les abonnements sont renouvelés automatiquement chaque mois</li>
          <li>L'essai gratuit de 7 jours est disponible sur les offres payantes</li>
          <li>La résiliation peut être effectuée à tout moment depuis votre espace</li>
          <li>Aucun remboursement n'est accordé pour les périodes déjà facturées</li>
        </ul>
      </div>

      <div className="cgu-section">
        <div className="cgu-h2">8. Modification des conditions</div>
        <p className="cgu-p">LegitWear se réserve le droit de modifier les présentes CGU à tout moment. Les utilisateurs seront informés des modifications importantes par email.</p>
      </div>

      <div className="cgu-section">
        <div className="cgu-h2">9. Droit applicable</div>
        <p className="cgu-p">Les présentes CGU sont soumises au droit français. En cas de litige, les tribunaux français seront seuls compétents.</p>
      </div>
    </div>
  );
}
...
export default function App() {
  ...
  return (
    <>
      ...
      />
      {page === "landing" && <Landing onLogin={() => goAuth("login")} onSignup={() => goAuth("signup")} onShare={() => setShowShare(true)} onCheckout={handleCheckout} onCGU={() => setPage("cgu")} />}
      ...
      {page === "dashboard" && user && <Dashboard userEmail={user} onShare={() => setShowShare(true)} />}
